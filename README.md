...
| 內餡 | 加料 | 顆數/盒 |
|---|---|---|
| 紅豆、芋頭、綠豆、巧克力 | 原味、鹹蛋黃、麻薯、肉鬆 | 六顆 |

任何內餡同價；原味每顆 $50，鹹蛋黃／麻薯／肉鬆每顆 $55。單純顆數單價 × 6，**沒有額外的禮盒加價**。（原本還有十二顆裝，2026-09-15 拿掉了；後端 `PACK_SIZES` 也同步只剩 `[6]`。）
...
`admin.html` 的訂單清單，每張訂單卡片下方有：
- **已收款** checkbox：勾了會即時回寫試算表，卡片變綠色方便一眼看出誰還沒收錢
  （`doPost action:"admin_setPaid"` + `orderId` + `paid`；同一張訂單的每個品項列會一起更新）。
- **🗑 刪除訂單**：整張訂單（所有品項列）一次刪掉，會先跳確認視窗
  （`doPost action:"admin_deleteOrder"` + `orderId`），**刪了不能復原**，沒有回收桶。
- 試算表「訂單明細」多一欄「已收款」（放在最後面，跟單顆價格/小計金額同一批加法：不影響舊資料欄位順序）。
...
| 時間戳記 | 訂單編號 | 訂購人 | 電話 | 取貨日期 | 取貨時段 | 內餡 | 加料 | 顆數 | 盒數 | 總顆數 | 備註 | 單顆價格 | 小計金額 | 已收款 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|

> 「單顆價格」「小計金額」「已收款」都是後來加的欄位，刻意放最後面，不會打亂舊資料既有欄位順序；
> 舊訂單（加價格前下的單）單顆價格/小計金額這兩格是空的，admin 頁讀到空值會自動用目前定價規則現算，不會顯示壞掉的數字。
> 「已收款」是 `TRUE`/空白，同一張訂單的每一列都會同步寫，在 admin 頁勾選即可。
